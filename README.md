[![Open in Visual Studio Code](https://classroom.github.com/assets/open-in-vscode-2e0aaae1b6195c2367325f4f02e2d04e9abb55f0b24a779b69b11b9e10269abc.svg)](https://classroom.github.com/online_ide?assignment_repo_id=21146824&assignment_repo_type=AssignmentRepo)
# WordFind
<!-----
...
See the (Adobe Acrobat) ***WordFind.pdf*** file for a basic description of the problem. Below are clarifications and specific requirements.
...
If the second command line argument isn't present, prompt for a word to search for, and read a single word/phrase from the keyboard. Print the results as in the example, and then prompt the user for another word. Continue prompting and searching until no word is entered – until the user just presses Enter after the prompt – then exit the program.

The word-find grid will have *r* rows and *c* columns. The format of the word-find file grid will be 2*r*+1 strings, each on a separate line, and each string of length 2*c*+1. You may assume *r* and *c* are each greater than zero and less than 100, but *r* and *c* are not necessarily equal. Each line will be separated by a line of hyphens (-), and each letter on a line will be separated by a vertical bar (|). Note you may be asked to find a *phrase,* multiple words separated by the space character. These phrases appear in the grid without spaces. (But, in your output, it's best to print the phrase *with* spaces.)
